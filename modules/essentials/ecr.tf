resource "aws_ecr_pull_through_cache_rule" "cache" {
  for_each = var.configure_ecr_pull_through && var.configure_ecr_pull_through_policy ? var.ecr_pull_through_cache_rules : {}
...
data "aws_iam_policy_document" "ecr_cache" {
  count = var.configure_ecr_pull_through_policy ? 1 : 0

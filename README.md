...
| <a name="input_migrate_aws_auth_to_access_entry"></a> [migrate\_aws\_auth\_to\_access\_entry](#input\_migrate\_aws\_auth\_to\_access\_entry) | Migrate existing aws-auth ConfigMap entries to access entries, Note - Set to `true` when you need to change authentication mode from `CONFIG_MAP` to `API_AND_CONFIG_MAP` | `bool` | `false` | no |
